 ...
 * @date 2016-08-18
 */
public class MybatisSqlReservedWords {
     ...
     */
    private MybatisSqlReservedWords() {
     ...
     * @return
     */
    public static String convert(MybatisGlobalAssistant globalAssistant, String column) {
        if (globalAssistant.getDbType() == DBType.POSTGRE) {
            // POSTGRE 直接转换
            return convertQuote(globalAssistant, column);
        }
        return containsWord(column) ? convertQuote(globalAssistant, column) : column;
    }

    public static String convertQuote(MybatisGlobalAssistant globalAssistant, String column) {
        String identifierQuote = globalAssistant.getIdentifierQuote();
